        ...
        <View style={styles.headerIconContainer}>
          <Image
            ...
            style={[
              ...
              {tintColor: '#3029b4ff'}, // change color here
            ]}
          />
        </View>
      </View>
...
const AddStudentScreen = () => {
  ...
  const fetchStudents = useCallback(async () => {
    try {
      ...
      for (const doc of snapshot.docs) {
        ...
        const avatar =
          data.photoURL || `https://i.pravatar.cc/150?u=${data.email}`;
  ...
  const organizeSections = (students: Student[], query: string) => {
    ...
    if (available.length > 0)
      newSections.push({title: 'Available Students', data: available});
    if (assigned.length > 0)
      newSections.push({title: 'Already Assigned', data: assigned});
  ...
  const handleAddStudent = async (studentId: string) => {
    ...
    try {
      ...
      const updatedList = allStudents.map(s =>
        s.id === studentId
          ? {...s, counselorId: currentCounselorId, counselorName: 'You'}
          : s,
      );
          ...
          <View style={styles.infoContainer}>
            ...
            {isAssigned && (
              <View
                style={[
                  styles.statusBadge,
                  isAssignedToMe ? styles.badgeMine : styles.badgeOthers,
                ]}>
                <Text
                  style={[
                    styles.statusText,
                    isAssignedToMe ? styles.textMine : styles.textOthers,
                  ]}>
                  {isAssignedToMe
                    ? 'Assigned to You'
  ...
  return (
    <View style={styles.container}>
      ...
      <ModernHeader
        title="Manage Class"
        subtitle="Add new students to your list"
      />
